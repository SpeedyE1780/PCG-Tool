...
namespace pcg::engine::c_api
{
    ...
    void simpleGeneration(const level_generation::GenerationData& data, const math::Vector3& offset, addNodeCallback callback)
    {
    ...
    void waveFunctionCollapse4DGeneration(const level_generation::GenerationData& data, level_generation::ExpansionMode mode, math::Axis axes, addWFC4DNodeCallback callback)
    {
    ...
    void generateCyclicSequence(int nextNodeCount, int sequenceLength, updateSequence updateSequence)
    {
        SequenceNodeWrapper::setCallbacks(updateSequence);
        SequenceNodeWrapper wrappedNode(nextNodeCount);
        combination_generation::generateSequence(wrappedNode, sequenceLength, nullptr);
        //Dirty fix to invoke update sequence on last node in sequence
        updateSequence(-1);
        SequenceNodeWrapper::resetCallbacks();
    }
